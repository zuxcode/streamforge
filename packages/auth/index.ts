...
import type { AuthenticatedUser, SubscriptionStatus, UserRole } from '@streamforge/types';
...
const VALID_SUBSCRIPTIONS = new Set<string>(['active', 'inactive', 'trialing', 'past_due']);
...
export function normaliseUserProfile(
  ...
  if (!role || typeof role !== 'string' || !VALID_ROLES.has(role)) {
    throw new UnauthenticatedError(
      `Unrecognised role: "${role}". Expected one of: ${[...VALID_ROLES].join(', ')}.`,
    );
  }
...
export async function verifyJwt(
  ...
  try {
    decoded = (await verify(token, config.publicKey, 'HS256')) as Record<string, unknown>;
...
export function createAuthMiddleware(options: CreateAuthMiddlewareOptions): MiddlewareHandler {
  const cache = new TokenCache(options.cacheTtlSeconds ?? 30);

  const evictionInterval = setInterval(() => cache.evictExpired(), 5 * 60 * 1000);
  if (typeof evictionInterval === 'object' && 'unref' in evictionInterval) {
    evictionInterval.unref();
  }

  return async function authMiddleware(c: Context, next: Next): Promise<void | Response> {
...
export function requireActiveSubscription(user: AuthenticatedUser): void {
  ...
  if (!allowed.includes(user.subscription)) {
    throw new UnauthorizedError('An active subscription is required to stream video.');
  }
}
