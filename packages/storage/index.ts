...
export const s3Keys = {
   ...
   */
  manifest: (filename: string): string => join('processed', filename, 'master.m3u8'),

  /**
   * Key for the HLS manifest.
   * e.g. processed/abc-123/thumbnail.jpg
   */
  thumbnail: (filename: string): string => join('processed', filename, 'thumbnail.jpg'),
...
export function createStorageClient(config: StorageClientConfig): StorageClient {
  ...
  async function upload(
    ...
    try {
      ...
      const writer = file.writer(options.contentType ? { type: options.contentType } : {});
      ...
      throw new StorageError(`Failed to upload to S3 key: ${key}`, key, err);
    }
  }
  ...
  // uploadFile
  // -------------------------------------------------------------------------
  async function uploadFile(key: string, sourcePath: string, contentType?: string): Promise<void> {
    try {
      ...
      if (err instanceof StorageError) throw err;
      throw new StorageError(`Failed to upload file "${sourcePath}" to S3 key: ${key}`, key, err);
    }
  }
  ...
  // download
  // -------------------------------------------------------------------------
  async function download(key: string, options: DownloadOptions): Promise<void> {
    try {
      const file = getFile(key);
      ...
      if (err instanceof StorageError) throw err;
      throw new StorageError(`Failed to download S3 key: ${key}`, key, err);
    }
  }
  ...
  // getStream
  // -------------------------------------------------------------------------
  async function getStream(key: string): Promise<{
    meta: Bun.S3Stats;
    stream: ReadableStream<Uint8Array<ArrayBuffer>>;
  }> {
    try {
      ...
      throw new StorageError(`Failed to open stream for S3 key: ${key}`, key, err);
    }
  }
  ...
  async function exists(key: string): Promise<boolean> {
    try {
      ...
      throw new StorageError(`Failed to check existence of S3 key: ${key}`, key, err);
    }
  }
  ...
  // delete
  // -------------------------------------------------------------------------
  async function deleteFn(prefix: string): Promise<void> {
    try {
      const files = await getS3Client().list({ prefix });

      const contents = files.contents;

      // Recursive delete should be idempotent
      if (!contents?.length) {
        return;
      }

      const keys = contents.map((file) => file.key).filter((key): key is string => Boolean(key));

      const results = await Promise.allSettled(
        keys.map(async (objectKey) => {
          try {
            await getFile(objectKey).delete();
          } catch (err) {
            // S3 DELETE is idempotent — a 404 is not an error

            if (!isNotFoundError(err)) {
              throw new StorageError(`Failed to delete S3 key: ${objectKey}`, objectKey, err);
            }
          }
        }),
      );
      ...
      throw new StorageError(`Failed to delete S3 prefix: ${prefix}`, prefix, err);
    }
  }
