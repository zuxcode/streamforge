...
async function guardLesson(
  ...
  if (
    ...
    throw c.json<ErrorResponse>(
      { error: { code: "DENIED", message: "Missing access token" } },
      404,
    );
  }
  const user = c.get("user");
  const payload = payloadClient({ strategy: "token", token });
  ...
  if (
    !lesson || typeof lesson.module !== "object" ||
  ...
  const courseId = typeof lesson.module.course !== "object"
...
streamRoute.get(
  "/stream/:lessonId/processed/:directory/master.m3u8",
  async (c) => {
...
streamRoute.get(
  "/stream/:lessonId/processed/:directory/:quality/:filename",
  async (c) => {
    const lessonId = c.req.param("lessonId");
