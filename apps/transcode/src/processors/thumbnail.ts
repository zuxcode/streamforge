...
//     `Video duration: ${
//       duration.toFixed(1)
//     }s — generating ${count} thumbnail(s)`,
//   );
...
//       const seekSec = (duration * offset).toFixed(3);
//       const filename = `thumb_${String(i + 1).padStart(2, "0")}.jpg`;
//       const outPath = join(outputDir, filename);

//       logger.info(
//         `Generating thumbnail ${i + 1}/${count} at ${seekSec}s → ${filename}`,
//       );
...
//         const tail = stderr.split("\n").slice(-10).join("\n");
//         throw new Error(
//           `ffmpeg exited with code ${code} for frame ${i + 1}:\n${tail}`,
//         );
//       }
...
async function getDuration(inputPath: string): Promise<number> {
  try {
    ...
    logger.error({ error, inputPath, context: 'getDuration' }, 'failed to get video duration');
    throw error;
  }
}
...
export interface ThumbnailOptions {
  ...
  outputDir: string;
  /**
   * Number of thumbnails to extract. Default: 1 (a single frame at 50% of
   * the video's duration). For count > 1, frames are spread from 15% to
   * 85% of the duration to avoid black frames at the very start and end.
   */
  count?: number;
  /** Thumbnail width in pixels (height scaled automatically). Default: 640 */
  width?: number;
...
/**
 * Extracts one or more evenly spaced JPEG thumbnails from a video file.
 *
 * With the default `count: 1`, a single thumbnail is taken at 50% of the
 * video's duration. For `count > 1`, thumbnails are spread from 15% to 85%
 * of the duration to avoid black frames at the very start and end. Each
 * frame is extracted by a separate Bun.spawn("ffmpeg") call so failures
 * are isolated per frame.
 *
 * Note: if a later frame in a multi-thumbnail batch fails, any thumbnails
 * already written for earlier frames are left on disk and not cleaned up
 * here — the function throws without returning paths. Callers relying on
 * a temp-directory-wide cleanup elsewhere (e.g. after the job completes
 * or fails) are unaffected; callers without that safety net should clean
 * up `outputDir` themselves on error.
 *
 * @returns Absolute paths to the generated thumbnail files.
 *
 * @example
...
export async function generateThumbnails({
  ...
  try {
    ...
    logger.error(
      { error, outputDir, context: 'generateThumbnails' },
  ...
  const duration = await getDuration(inputPath);
  logger.info(
    { durationSec: Number(duration.toFixed(1)), count },
    'video duration determined — generating thumbnails',
  );

  // 15 % … 85 % spread avoids black frames at the edges
  const offsets =
    count === 1 ? [0.5] : Array.from({ length: count }, (_, i) => 0.15 + (i * 0.7) / (count - 1));
  ...
  try {
    for (const [i, offset] of offsets.entries()) {
      ...
      logger.info({ frame: i + 1, count, seekSec, filename }, 'generating thumbnail');
    ...
    logger.info({ count: thumbnailPaths.length }, 'thumbnails generated');
    return thumbnailPaths;
  } catch (error) {
    logger.error(
      { error, generated: thumbnailPaths.length, requested: count, context: 'generateThumbnails' },
      'failed to generate thumbnails',
    );
    throw error;
  }
