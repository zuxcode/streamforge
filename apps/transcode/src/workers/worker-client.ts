...
async function fireWebhook(url: string, payload: FireWebhookPayload) {
  ...
  try {
    ...
    if (!res.ok) {
      const error = await res.json().catch((e) => logger.error(e));
      logger.warn(
        {
          url,
          status: res.status,
...
export function createTranscodeWorker(): Worker<TranscodeJob> {
  if (!worker) {
    ...
    worker.on('completed', (job, result) => {
      ...
      if (transcodeEnv.TRANSCODE_WEBHOOK_PATH) {
        void fireWebhook(transcodeEnv.TRANSCODE_WEBHOOK_PATH, {
          event: 'job.complete',
    ...
    worker.on('failed', (job, err) => {
      ...
      if (!job || !transcodeEnv.TRANSCODE_WEBHOOK_PATH) return;
